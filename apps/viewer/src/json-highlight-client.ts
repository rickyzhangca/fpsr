...
export class JsonHighlightWorkerClient {
  private readonly pending = new Map<
    number,
    {
      resolve(lines: JsonHighlightToken[][]): void;
...
const getWorkerClient = (): JsonHighlightWorkerClient => {
  if (!workerClient) {
    const worker = new Worker(new URL("./json-highlight.worker.ts", import.meta.url), {
      type: "module",
    });
...
export const highlightJsonPage = (code: string): Promise<JsonHighlightToken[][]> => {
