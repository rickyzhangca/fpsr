...
import { updateActiveRenderProgress } from "./render-progress-state";
describe("updateActiveRenderProgress", () => {
  it("replaces the renderer's complete event with the timed completion event", () => {
    ...
    const complete = updateActiveRenderProgress(null, { value: 100, label: "Complete" }, selection);
    const timed = updateActiveRenderProgress(
      complete,
      { value: 100, label: "Complete", durationMs: 1234 },
      selection,
    );
